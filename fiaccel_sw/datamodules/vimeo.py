...
from datamodules.Vimeo90kSeptuplet import Vimeo90kSeptuplet
from datamodules.Vimeo90kTriplet import Vimeo90kTriplet
...
class VimeoDataModule(LightningDataModule):
            ...
            augmentations = [
                RandomChoice(
                    [
                        ...
                        RandomResizedCrop(size=self.video_crop_size, scale=(0.6, 1),antialias=True),
                    ]
                ),
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        if self.name == 'Vimeo90kTriplet':

            self.train_dataset = Vimeo90kTriplet(
                root=self.data_dir,
                as_video=True,
            ...
            self.val_dataset = Vimeo90kTriplet(
                root=self.data_dir,
                as_video=True,
                tensor_transform=self._video_transform(mode="test"),
                split="test",
            )
            self.test_dataset = Vimeo90kTriplet(
                root=self.data_dir,
                as_video=True,
                tensor_transform=self._video_transform(mode="test"),
                split="test",
        ...
        elif self.name == 'Vimeo90kSeptuplet' :
