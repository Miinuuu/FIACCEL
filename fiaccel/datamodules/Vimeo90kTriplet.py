...
class Vimeo90kTriplet(Dataset):
    ...
    Args:
        ...
        frames_per_group: The number of frames to include from
            ...
            the dataset. Must be between 1 and 3.
        ...
        folder_list: A list of paths to septuplets to include in the dataset
            ...
            ``im1.png``, ..., ``im3.png``. Exactly one of ``split`` or
            ``folder_list`` must be specified.
    ...
    def __init__(
        ...
        if frames_per_group not in list(range(1, 3+1)):
            raise ValueError(
                "frames_per_group must be an integer between 1 and 3 (inclusive), "
        ...
        if folder_list is not None:
            ...
            with open(self.root / f"tri_{split}list.txt", "r") as file:
                self.folder_list = [
